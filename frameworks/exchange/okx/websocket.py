...
from frameworks.exchange.okx.exchange import Okx
from frameworks.exchange.okx.endpoints import OkxEndpoints
from frameworks.exchange.okx.ws_handlers.orderbook import OkxOrderbookHandler
from frameworks.exchange.okx.ws_handlers.trades import OkxTradesHandler
from frameworks.exchange.okx.ws_handlers.ticker import OkxTickerHandler
from frameworks.exchange.okx.ws_handlers.ohlcv import OkxOhlcvHandler
from frameworks.exchange.okx.ws_handlers.orders import OkxOrdersHandler
from frameworks.exchange.okx.ws_handlers.position import OkxPositionHandler


class OkxWebsocket(WebsocketStream):
    """
    Handles Websocket connections and data management for Okx.
    """

    def __init__(self, exch: Okx) -> None:
        super().__init__()
        self.exch = exch
        self.endpoints = OkxEndpoints()

    def create_handlers(self) -> None:
        self.public_handler_map = {
            "orderbook": OkxOrderbookHandler(self.data),
            "publicTrade": OkxTradesHandler(self.data),
            "kline": OkxOhlcvHandler(self.data),
            "tickers": OkxTickerHandler(self.data),
        }

        self.private_handler_map = {
            "order": OkxOrdersHandler(self.data, self.symbol),
            "position": OkxPositionHandler(self.data, self.symbol),
        }
    ...
    async def public_stream_handler(self, recv: Dict) -> None:
        try:
            ...
            await self.logging.error(f"Error with Okx public ws handler: {e}")
    ...
    async def start_public_stream(self) -> None:
        ...
        try:
            ...
            await self.logging.error(f"Okx Public Ws: {e}")
    ...
    async def start_private_stream(self):
        ...
        try:
            ...
            await self.logging.error(f"Okx Private Ws: {e}")
